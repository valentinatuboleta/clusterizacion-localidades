...
axes[0, 1].set_title("Peso de Aforo (% Recinto)", fontsize=10, fontweight="bold")
...
for i, r in top_venues_unica.iterrows():
    tot = r["Total"]
    pct = r["Pct"]
    plt.text(tot + 30, i, f"{tot:,} ({pct}%)", va="center", fontsize=8.5)
plt.xlim(0, max(top_venues_unica["Total"]) * 1.18)
plt.tight_layout()
plt.savefig("reports/figures/fig2b_top_venues_unica.png", dpi=200)
plt.close()

# 4. FIG 2C: Localidades Más Costosas Multi-Zona (Composición de Tags)
df_ratio_1_multi = df_enriquecido[(df_enriquecido["ratio_precio_max"] == 1.0) & (df_enriquecido["peso_aforo"] < 0.99)]
tag_counts_multi = df_ratio_1_multi[tag_cols].sum().sort_values(ascending=False).head(10).reset_index()
tag_counts_multi.columns = ["Tag", "Total"]
tag_counts_multi["Tag"] = tag_counts_multi["Tag"].str.replace("tag_", "").str.upper()

plt.figure(figsize=(9, 6.5))
sns.barplot(data=tag_counts_multi, y="Tag", x="Total", hue="Tag", palette="crest", legend=False)
plt.title("Atributos NLP en Localidades Más Costosas (Ratio = 1.0 en Multi-Zona, N=4,407)", fontsize=11, fontweight="bold")
plt.xlabel("Frecuencia de Ocurrencia")
for i, r in tag_counts_multi.iterrows():
    tot = r["Total"]
    pct = round(tot / len(df_ratio_1_multi) * 100, 1)
    plt.text(tot + 20, i, f"{tot:,} ({pct}%)", va="center", fontsize=8.5)
plt.xlim(0, max(tag_counts_multi["Total"]) * 1.18)
plt.tight_layout()
plt.savefig("reports/figures/fig2c_multizona_tags_top.png", dpi=200)
plt.close()

# 5. FIG 2D: Distribuciones Multi-Zona (Gráfico 2B del notebook)
...
# 6. FIG 3A: Demostración 17 Tags (Volumen y Discriminación)
dimensiones_nlp = {
...
# 7. FIG 3B: Boxplots Bivariados 7 Tags
...
print("All 9 figures generated and saved in reports/figures/ successfully!")
